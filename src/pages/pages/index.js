import * as Artists     from 'pages/pages/artists/Component';
import * as Atlas       from 'pages/pages/atlas/Component';
import * as Blog        from 'pages/pages/blog/Component';
import * as Genres      from 'pages/pages/genres/Component';
import * as Home        from 'pages/pages/home/Component';
import * as Info        from 'pages/pages/info/Component';
import * as Mode        from 'pages/pages/mode/Component';
import * as Shop        from 'pages/pages/shop/Component';

export {Artists, Atlas, Blog, Genres, Home, Info, Mode, Shop};